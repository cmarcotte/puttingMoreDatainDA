...
function errorPlot(workDir, obsDir; zs=[1,50], ens="gues", dx=0.015, dt=2.0)
	...
	fig, axs = plt.subplots(2, 2, figsize=(SW, SW), gridspec_kw=Dict("width_ratios"=> [5, 1], "wspace"=>0.01), sharex="col", sharey=true, constrained_layout=true)
	labels = ["EPI", "ENDO"]
	labs = ["(a)", "(b)"]
	for (o,z) in enumerate(zs)
		axs[o,1].fill_between(times,  errrms[:,o] .- errstd[:,o],  errrms[:,o] .+ errstd[:,o], color="C0", linewidth=0, alpha=0.3, label="")
		axs[o,1].fill_between(times,  sprrms[:,o] .- sprstd[:,o],  sprrms[:,o] .+ sprstd[:,o], color="C1", linewidth=0, alpha=0.3, label="")
		axs[o,1].plot(times, errrms[:,o],  "-C0", label="RMS Error")
		axs[o,1].plot(times, sprrms[:,o], "-C1", linewidth=1, label="RMS Spread")
		axs[o,1].set_ylabel("$(labels[o])")
		axs[o,1].text(-0.2, +1.1, labs[o], horizontalalignment="center", verticalalignment="center", transform=axs[o,1].transAxes)
		axs[o,2].hist(errrms[:,o], density=true, histtype="stepfilled", bins="auto", orientation="horizontal")
		axs[o,2].hist(sprrms[:,o], density=true, histtype="step", bins="auto", orientation="horizontal")
		axs[o,2].set_xticks([])
	end
	axs[1,1].legend(loc="lower left", bbox_to_anchor= (0.0, 1.01), ncol=2, borderaxespad=0, frameon=false)
	axs[2,1].set_xlabel("\$t\$ [ms]")
	axs[2,1].set_xlim([0,2000])
	axs[1,1].set_ylim([0.0,1.0])
	
	plt.savefig("$(workDir)_SurfaceErrorsHist.pdf", bbox_inches="tight", dpi=300)
	plt.close()
