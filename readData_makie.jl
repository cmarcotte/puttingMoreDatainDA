...
#=
using Main.readData, GLMakie, Printf, ProgressBars
...
record(fig, "volume.mp4", 1:124; framerate=20) do t
	fnames = [@sprintf("/home/chris/Development/Fortran/cardiacDA/3D/DATA/nature_4cm_straight_60degree_2/%04d/restart3d.%03d",t,p) for p in 0:3]
...
record(fig, "heatmaparray.mp4", 1:124; framerate=10) do t
	fnames = [@sprintf("/home/chris/Development/Fortran/cardiacDA/3D/DATA/nature_4cm_straight_60degree_2/%04d/restart3d.%03d",t,p) for p in 0:3]
	readRestarts!(u, v, w, fnames)
...
gus = [Observable(gu[z,:,:]) for z in Zs]
aus = [Observable(au[z,:,:]) for z in Zs]
dus = [Observable(au[z,:,:].-gu[z,:,:]) for z in Zs]
...
for j in 1:length(Zs)
	heatmap!(axs[1,j],gus[j],colormap="Oranges",colorrange=(0,1))
	heatmap!(axs[2,j],dus[j],colormap="bwr",colorrange=(-0.1,0.1))
	heatmap!(axs[3,j],aus[j],colormap="Oranges", colorrange=(0,1))
	axs[1,j].title = L"z=%$(Zs[j])"
end
Colorbar(fig[1,length(Zs)+1]; colorrange=(0,1), colormap="Oranges", label=L"u^b")
Colorbar(fig[2,length(Zs)+1]; colorrange=(-0.1,0.1), colormap="bwr", label=L"u^a-u^b")
Colorbar(fig[3,length(Zs)+1]; colorrange=(0,1), colormap="Oranges",  label=L"u^a")

resize_to_layout!(fig)
save("heatmap_array.png", fig)

# create video directly; blurrier, faster (if using GLMakie backend)
record(fig, "innovationMovieMakie.mp4", ProgressBar(0:124); framerate=10) do t
	fnames = [@sprintf("/home/chris/Development/Fortran/cardiacDA/3D/DATA/nature_4cm_straight_60degree_2/%04d/restart3d.%03d",t,p) for p in 0:3]
...
# create frames, then create video using ffmpeg; should be sharper but is not
for t in ProgressBar(0:124)
	fnames = [@sprintf("/home/chris/Development/Fortran/cardiacDA/3D/DATA/nature_4cm_straight_60degree_2/%04d/restart3d.%03d",t,p) for p in 0:3]
	readRestarts!(gu, gv, gw, fnames)
	fnames = [@sprintf("/home/chris/Development/Fortran/cardiacDA/3D/DATA/nature_4cm_straight_60degree_2/%04d/restart3d.%03d",t,p) for p in 0:3]
	readRestarts!(au, av, aw, fnames)
	...
	save(@sprintf("./innovation_frames_makie/%04d.png",t), fig)
end
rm("./innovationMovieMakie.mp4");
run(`ffmpeg -r 10 -f image2 -start_number 0 -i ./innovation_frames_makie/%04d.png -c:v h264_nvenc -pix_fmt yuv420p -loglevel quiet -stats ./innovationMovieMakie.mp4`);
